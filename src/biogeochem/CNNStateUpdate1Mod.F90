    ...
    associate(                                                                   & 
      ...
      do fp = 1,num_soilp
         ...
         if (ivt(p) >= npcropmin) then ! Beth adds retrans from froot
            ...
            ns_veg%grainn_patch(p)       = ns_veg%grainn_patch(p)     - nf_veg%grainn_to_biofuel_patch(p)*dt
            ns_veg%leafn_patch(p)        = ns_veg%leafn_patch(p)      - nf_veg%leafn_to_biofuel_patch(p)*dt
            ns_veg%livestemn_patch(p)    = ns_veg%livestemn_patch(p)  - nf_veg%livestemn_to_biofuel_patch(p)*dt
         end if
