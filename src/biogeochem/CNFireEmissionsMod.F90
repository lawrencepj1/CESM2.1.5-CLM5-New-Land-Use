    ...
    if (shr_fire_emis_mechcomps_n>0) then
       ...
       emis_cmp_loop: do while(associated(emis_cmp))
          ...
          call hist_addfld1d (fname='FireComp_'//trim(emis_cmp%name), units=units, &
               ...
               ptr_patch=this%comp(icomp)%emis, default='active')
